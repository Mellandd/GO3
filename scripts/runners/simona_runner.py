"""simona (R/Bioconductor) runner.

Wraps `scripts/runners/r_helpers/benchmark_simona.R`. Annotation is
provided as a TSV (gene<TAB>GO<TAB>ONTOLOGY) generated from the user's
GAF, identical to the one used by GOSemSim, so IC is consistent across
libraries.
"""
...
R_HELPER = Path(__file__).resolve().parent / "r_helpers" / "benchmark_simona.R"
...
class SimonaRunner(Runner):
    ...
    def caps(cls) -> RunnerCaps:
        return RunnerCaps(
            ...
            notes="Subprocess Rscript; user-supplied OBO + GAF-derived annotation TSV.",
        )
    ...
    def loading(cls, obo: Path, gaf: Path, namespace: str, *, python_executable: str | None = None) -> dict[str, Any]:
        ...
        return run_rscript_loading(
            ...
            namespace=namespace,
            measure="Resnik",
            extra_args=["--obo", str(obo), "--anno-tsv", str(anno_tsv)],
        )
